...
import { breadcrumb, escapeHtml, formatDate, layout, searchAssets, SITE_URL, styleAsset, visitorAsset } from './lib/html.mjs';
...
export function buildSite() {
  ...
  write('privacy/index.html', policyPage('개인정보처리방침', '버림뚝딱의 현재 개인정보 처리 범위와 외부 링크 정책을 안내합니다.', '/privacy/', `<p>버림뚝딱은 서비스 운영에 필요한 범위에서 최소한의 정보만 처리하는 것을 원칙으로 합니다.</p><h2>현재 처리하는 정보</h2><p>현재 사이트는 회원가입, 로그인과 외부 분석 도구를 사용하지 않습니다. Google AdSense 검토 및 광고 제공 준비를 위해 공식 AdSense 스크립트를 불러오며, Google의 데이터 처리는 Google의 정책에 따릅니다. 현재 사이트에는 AdSense 광고 슬롯이나 Auto ads를 활성화하지 않았습니다. 홈과 공식 자료로 확인된 품목 상세에는 쿠팡 파트너스 캐러셀 배너를 표시하며, 해당 배너는 쿠팡의 외부 스크립트를 불러옵니다. 사이트 검색은 브라우저에서 처리합니다. 검색 결과가 실제로 없는 경우에만 품목 추가 우선순위를 정하기 위해 정리된 검색어, 검색 횟수, 최초·최근 검색 시각을 집계할 수 있습니다.</p><p>footer의 TODAY는 한국 날짜별 익명 브라우저 탭 세션 방문 횟수로, 정확한 고유 사용자 수가 아닌 참고 수치입니다. 같은 탭에서 중복 증가를 막기 위해 sessionStorage에 해당 날짜의 임시·완료 표시만 보관합니다. 이 정보는 탭 세션 범위에서만 사용하며 localStorage나 식별용 쿠키를 만들지 않습니다.</p><p>버림뚝딱 애플리케이션 데이터베이스에는 이름, 이메일 주소, IP 주소, User-Agent 원문, 쿠키 식별자, fingerprint 또는 정확한 위치를 저장하지 않습니다. TODAY 집계에는 날짜·횟수·마지막 갱신 시각만 저장합니다. Cloudflare와 쿠팡 등 외부 서비스의 인프라 수준 데이터 처리는 각 서비스의 정책에 따라 별도로 처리될 수 있습니다.</p><h2>외부 링크</h2><p>정부기관과 폐가전 수거기관, 쿠팡 등 외부 사이트 링크를 열면 해당 사이트의 개인정보처리방침이 적용됩니다.</p><h2>문의와 시행일</h2><p>운영자 연락 경로와 공개 시행일은 정식 공개 전에 확정해 반영합니다.</p>`));
