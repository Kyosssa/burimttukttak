...
export function buildSite() {
  ...
  write('privacy/index.html', policyPage('개인정보처리방침', '버림뚝딱의 현재 개인정보 처리 범위와 외부 링크 정책을 안내합니다.', '/privacy/', `<p>버림뚝딱은 서비스 운영에 필요한 범위에서 최소한의 정보만 처리하는 것을 원칙으로 합니다.</p><h2>현재 처리하는 정보</h2><p>현재 사이트는 회원가입, 로그인과 외부 분석 도구를 사용하지 않습니다. Google AdSense 검토 및 광고 제공 준비를 위해 공식 AdSense 스크립트를 불러오며, Google의 데이터 처리는 Google의 정책에 따릅니다. 현재 사이트에는 광고 슬롯이나 Auto ads를 활성화하지 않았습니다. 사이트 검색은 브라우저에서 처리합니다. 검색 결과가 실제로 없는 경우에만 품목 추가 우선순위를 정하기 위해 정리된 검색어, 검색 횟수, 최초·최근 검색 시각을 집계할 수 있습니다.</p><p>버림뚝딱 애플리케이션 데이터베이스에는 이름, 이메일 주소, IP 주소, User-Agent 원문, 쿠키 식별자, fingerprint 또는 정확한 위치를 저장하지 않습니다. Cloudflare 인프라 수준의 로그는 Cloudflare의 정책에 따라 별도로 처리될 수 있습니다.</p><h2>외부 링크</h2><p>정부기관과 폐가전 수거기관 등 외부 사이트 링크를 열면 해당 사이트의 개인정보처리방침이 적용됩니다.</p><h2>문의와 시행일</h2><p>운영자 연락 경로와 공개 시행일은 정식 공개 전에 확정해 반영합니다.</p>`));
  write('affiliate-disclosure/index.html', policyPage('제휴 마케팅 안내', '버림뚝딱의 제휴 마케팅 적용 여부와 향후 고지 원칙을 안내합니다.', '/affiliate-disclosure/', `<p>현재 버림뚝딱에는 쿠팡을 포함한 제휴 링크와 상품 배너가 없습니다. Google AdSense 스크립트는 검토 및 광고 제공 준비를 위해 로드하지만, 현재 광고 슬롯과 Auto ads는 활성화하지 않았습니다.</p><p>향후 제휴 기능을 사용하게 되면 사용자가 링크를 누르기 전에 경제적 이해관계를 해당 제휴 영역 가까이에 분명하게 표시하고 이 안내를 갱신합니다.</p><p>제휴 링크를 통한 구매가 적용되는 경우 구매자에게 별도 비용을 더하지 않으면서 버림뚝딱이 수수료를 받을 수 있다는 사실도 링크 전에 안내합니다.</p>`));
