...
export const searchAssetVersion = createHash('sha256')
  .update(Buffer.concat(searchSources.map(path => readFileSync(new URL(path, import.meta.url)))))
  .digest('hex').slice(0, 12);
export const searchAssets = {
  app: `app-${searchAssetVersion}.js`,
  search: `search-${searchAssetVersion}.js`,
  reporter: `missing-search-${searchAssetVersion}.js`,
  index: `search-index-${searchAssetVersion}.json`,
};
...
export function layout({ title, description, canonical, content, mainClass = '', search = false, robots = 'index,follow', jsonLd = [] }) {
  ...
  const socialUrl = absoluteCanonical ?? `${SITE_URL}/404.html`;
  const social = `
  ${absoluteCanonical ? `<link rel="canonical" href="${escapeHtml(absoluteCanonical)}">` : ''}
  ...
  <meta property="og:image:alt" content="버림뚝딱 - 헷갈리는 물건, 어떻게 버리는지 바로 확인하세요">
  <meta name="twitter:card" content="summary">
  <meta name="twitter:title" content="${escapeHtml(title)}">
  <meta name="twitter:description" content="${escapeHtml(description)}">
  <meta name="twitter:image" content="${SITE_URL}/og-default.png">
  <meta name="twitter:image:alt" content="버림뚝딱 - 헷갈리는 물건, 어떻게 버리는지 바로 확인하세요">`;
  return `<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <header class="site-header"><div class="wide"><a class="brand" href="/" aria-label="버림뚝딱 홈"><img src="/logo-horizontal.svg" alt="버림뚝딱"></a><nav aria-label="주요 메뉴"><a href="/#categories">카테고리</a><a href="/source-policy/">정보 검증</a><a href="/about/">소개</a></nav></div></header>
